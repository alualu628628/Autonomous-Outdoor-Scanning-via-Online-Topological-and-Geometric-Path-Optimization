...
*************************************************/
	TopologyMap::TopologyMap(ros::NodeHandle & node,
	                         ...
	                         m_vGroundCloud(new pcl::PointCloud<pcl::PointXYZ>),
			...
			for (int i = 0; i != vOneGCloud.size(); ++i) {
				...
				if (!(i%m_iPCSmplNum)) {
					...
					if (CheckInSidePoint(vOneGCloud.points[i])) {
						//to point clouds
						m_vGroundCloud->points.push_back(vOneGCloud.points[i]);

						int iPointIdx = PointoOneDIdx(vOneGCloud.points[i]);
						//to point idx
						m_vGroundPntMapIdx[iPointIdx].push_back(m_iGroundFrames);
						m_iGroundFrames++;
...
void TopologyMap::ComputeTravelFeature(const pcl::PointXYZ & oRobotIdx) {
	...
	for (int i = 0; i != vNearByIdx.size(); ++i) {
		...
		for (int j = 0; j != m_vGroundPntMapIdx[vNearByIdx[i].iOneIdx].size(); ++j) {
			int iPointIdx = m_vGroundPntMapIdx[vNearByIdx[i].iOneIdx][j];
			vCloud.push_back(m_vGroundCloud->points[iPointIdx]);
		}
	}
